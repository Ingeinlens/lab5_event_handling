...
namespace WinFormsApp.Objects
{
    internal class BaseObject
    {
        ...
        // Метод с помощью которого можно описать форму объекта
        public virtual GraphicsPath GetGraphicsPath()
        {
            return new GraphicsPath();
        }

        //Проверяем пересечение двух форм
        public virtual bool Overlaps(BaseObject baseObject, Graphics g)
        {
            // берем информацию о форме
            var path1 = this.GetGraphicsPath();
            var path2 = baseObject.GetGraphicsPath();

            // применяем к объектам матрицы трансформации
            path1.Transform(this.GetTransform());
            path2.Transform(baseObject.GetTransform());
        ...
        public virtual void Overpal(BaseObject baseObject)
        {
            if (this.OnOverlap != null)
            {
                this.OnOverlap(this, baseObject);
            }
        }
    }
}